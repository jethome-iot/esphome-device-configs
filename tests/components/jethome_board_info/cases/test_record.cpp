...
// own magic, version, signature algorithm and CRC. The two CRCs are deliberately independent.
...
TEST(JeefsRecord, AcceptsEverySignatureAlgorithmInTheEnum) {
  for (uint8_t algorithm : std::array<uint8_t, 3>{0, 1, 2}) {
    auto rec = make_record(DEVID_RECORD_VERSION, algorithm);
    EXPECT_EQ(validate_record(rec.data(), file_data_crc_of(rec)), RecordStatus::OK)
        << "signature_version " << static_cast<unsigned>(algorithm);
  }
}

// Spec: a signature algorithm outside the enum is a parse error, never something to guess past.
TEST(JeefsRecord, RejectsASignatureAlgorithmOutsideTheEnum) {
  for (uint8_t algorithm : std::array<uint8_t, 4>{3, 4, 128, 255}) {
    auto rec = make_record(DEVID_RECORD_VERSION, algorithm);
...
// lands on SECP192R1, which is a legal algorithm, so only the stale record CRC catches it.
