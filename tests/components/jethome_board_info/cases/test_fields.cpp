// The field helpers: bounded-string extraction, MAC formatting, and the USID/serial
// cross-check that ties a record serial to the header's USID.
...
TEST(JeefsFields, FormatMacMatchesTheUpstreamVectors) {
  ...
  EXPECT_EQ(format_mac_plain(V3_HEADER_SIGNED.data() + HDR_OFF_MAC), "F0578DAABBCC");
}
