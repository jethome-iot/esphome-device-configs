...
struct Board {
  ...
  std::vector<uint8_t> signature() const {
    ...
    return std::vector<uint8_t>(sig, sig + SIGNATURE_SIZE);
  }
...
inline std::vector<uint8_t> v3_image() {
  ...
  memcpy(image.data(), V3_HEADER_SIGNED.data(), V3_HEADER_SIGNED.size());
  return image;
}

// A signed record, the signature bytes all `fill` so a test can tell it apart.
inline RecordBuf signed_record(uint8_t fill) {
  RecordBuf record = make_record(DEVID_RECORD_VERSION, SIG_CURRENT);
  memset(record.data() + REC_OFF_SIGNATURE, fill, SIGNATURE_SIZE);
  seal_record(record);
  return record;
}
...
inline std::vector<uint8_t> v4_image(const RecordBuf &record, uint8_t header_signature = SIG_NONE) {
  ...
  memset(image.data() + HDR_OFF_SIGNATURE, 0xCD, SIGNATURE_SIZE);
  reseal_header(image);
  return image;
}
