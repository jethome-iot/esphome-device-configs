...
TEST(BoardInfo, TheRecordsSignatureIsTheOneHandedOut) {
  Board board(v4_image(signed_record(0xAB), SIG_SECP256R1));
  board.boot();

  EXPECT_EQ(board.info.get_signature_version(), SIG_SECP256R1);
  EXPECT_EQ(board.signature(), std::vector<uint8_t>(ECDSA_P256_SIG_SIZE, 0xAB));
}

TEST(BoardInfo, ARecordWithoutASignatureIsNotCoveredByTheHeaders) {
  Board board(v4_image(make_record(), SIG_SECP256R1));
  board.boot();

  EXPECT_TRUE(board.info.has_device_identity());
  EXPECT_EQ(board.info.get_signature_version(), SIG_NONE);
  EXPECT_EQ(board.signature(), std::vector<uint8_t>(ECDSA_P256_SIG_SIZE, 0));
}
...
TEST(BoardInfo, AV3BoardTakesTheSerialAndTheSignatureFromTheHeader) {
  ...
  EXPECT_EQ(board.info.get_signature_version(), SIG_SECP256R1);
  std::vector<uint8_t> expected(ECDSA_P256_SIG_SIZE);
...
// The algorithm is reported as written, known or not: judging it is the verifier's job.
TEST(BoardInfo, AnUnknownHeaderAlgorithmIsReportedAsIs) {
...
// What a verifier outside the device has to hash: the string the format's signing_payload
// builds from the published fields and the chip's MACs.
TEST(BoardInfo, ThePublishedFieldsRebuildTheSignedPayload) {
  Board board(v3_image());
  board.boot();

  const std::string payload =
      signing_payload(board.info.get_cpuid(), format_mac_plain(board.info.get_mac()), board.info.get_usid());
  EXPECT_EQ(payload, "AA:BB:CC:DD:EE:FF:F0578DAABBCC:1234567890ABCDEF");
}
...
TEST(BoardInfo, ACorruptRecordIsNotAnIdentity) {
  std::vector<uint8_t> image = v4_image(signed_record(0xAB), SIG_SECP256R1);
  ...
  EXPECT_EQ(board.info.get_signature_version(), SIG_SECP256R1);
  EXPECT_EQ(board.signature(), std::vector<uint8_t>(ECDSA_P256_SIG_SIZE, 0xCD));
}
...
TEST(BoardInfo, ARecordSerialTheUsidDoesNotEncodeIsReported) {
  ...
  EXPECT_TRUE(board.log().has_warning("serial 'DSN-2026-000043' disagrees with the signed USID"));
}
