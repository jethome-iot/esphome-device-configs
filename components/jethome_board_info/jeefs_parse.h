...
// The last signature_version the format defines; the byte is reported as written.
static const uint8_t SIG_CURRENT = 2;
...
inline RecordStatus validate_record(const uint8_t *raw, uint32_t file_data_crc) {
  ...
  // Spec: a signature_version past the ones the format defines is a parse error, never something
  // to guess past.
  if (rec->signature_version > SIG_CURRENT) {
...
// The USID carries the serial's last 10 digits, so a record serial that disagrees with it is
// not the one production wrote.
