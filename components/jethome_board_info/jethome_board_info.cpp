  ...
  if (this->devid_valid_) {
    ...
    if (this->serial_checked_) {
      ...
      if (!this->serial_match_) {
        ESP_LOGW(TAG, "%s serial '%s' disagrees with the signed USID", DEVICE_ID_NAME, this->device_serial_.c_str());
      }
    }
  }
}
...
void JetHomeBoardInfo::log_record_status_(RecordStatus status) {
  switch (status) {
    ...
    case RecordStatus::UNSUPPORTED_SIGNATURE:
      ESP_LOGE(TAG, "Unsupported %s signature algorithm: %u", DEVICE_ID_NAME, this->devid_.signature_version);
      return;
...
void JetHomeBoardInfo::dump_config() {
  ...
  if (this->devid_valid_) {
    ...
    if (this->serial_checked_) {
      ESP_LOGCONFIG(TAG, "  Serial vs signed USID: %s", this->serial_match_ ? "MATCH" : "MISMATCH");
    }
  } else {
    ESP_LOGCONFIG(TAG, "  Device identity: ABSENT");
  }
  ESP_LOGCONFIG(TAG, "  Signature algo: %u", this->get_signature_version());
}
