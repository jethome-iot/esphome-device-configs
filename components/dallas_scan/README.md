...
`sensors:` hands the first slots to sensors declared in YAML; the scan fills the slots after
them:
...
others. A 1-Wire sensor in the list needs an `address:`; its device then keeps that slot, so
the scan does not hand it another one. Listed slots are `pinned(slot)`: forget leaves them
alone.
