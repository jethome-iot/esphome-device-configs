...
# installing that would replace the user's own firmware.
EXCLUDED_PACKAGES = {"firmware_update": ("fw_device",)}
...
def flatten(path: Path) -> dict[str, Any]:
    """Merge packages, skipping the substitution pass and the packages dist/ leaves out.
    ...
    for name in EXCLUDED_PACKAGES:
        (config.get("packages") or {}).pop(name, None)
...
def render(source: Path) -> tuple[Path, str] | None:
    ...
    for owned in EXCLUDED_PACKAGES.values():
        for key in owned:
...
def manifest_devices() -> dict[str, str]:
    """Each device config's slug on the firmware server, from firmwares.yaml."""
    data = yaml.safe_load((REPO_ROOT / MANIFEST_NAME).read_text())
    return {entry["config"]: entry["device"] for entry in data["firmwares"]}
