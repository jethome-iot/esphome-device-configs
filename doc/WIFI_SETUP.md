...
back with no network stored, so in `WiFi` or `Auto` mode the access point is up at once and
you reconfigure through the captive portal as above; in `Ethernet` mode WiFi stays off until
you switch the mode.
