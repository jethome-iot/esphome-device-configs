...
- leaves out the packages named in `EXCLUDED_PACKAGES` — `firmware-update`, because a firmware
  built from `dist/` is the importing user's own and the update entity would offer to replace it
  with JetHome's build;
