...
- **Board identity**: the model, hardware revision, board version, serial number and factory signature the factory wrote into the CPU board's EEPROM, read at boot for the log, the serial row of the **Info** menu and a verifier outside the device ([details](components/jethome_board_info/README.md))
